        ...
        for ($i = 1; $i <= $sheetCount; $i++) {
            ...
            foreach ($Reader as $Row) {
                ...
                if (!empty($INTERNAL_ITEM)) {
                    ...
                    $query_check = "SELECT INTERNAL_ITEM FROM `master_bom` WHERE INTERNAL_ITEM = '" . $INTERNAL_ITEM . "' AND MATERIAL_CODE = '" . $MATERIAL_CODE . "' ";
                    $result_check = mysqli_query($dbMi_138, $query_check);
                    if (mysqli_num_rows($result_check) > 0) {
                        $query = "UPDATE `master_bom` 
                        SET  `ITEM_DES` = '" . $ITEM_DES . "', `RBO` = '" . $RBO . "', `ORDERED_ITEM` = '" . $ORDERED_ITEM . "', `MATERIAL_DES` = '" . $MATERIAL_DES . "', `RIBBON_CODE` = '" . $RIBBON_CODE . "',
                        ...
                        WHERE INTERNAL_ITEM = '" . $INTERNAL_ITEM . "' AND MATERIAL_CODE = '" . $MATERIAL_CODE . "' ";
                    } else {
...
echo "<br />\n <div style='width:100%;text-align:center;color:red;font-weight:bold;'>$type. $message </div>";
echo "<br />\n <div style='width:100%;text-align:center;color:blue;'>Hệ thống sẽ load lại sau 5s</div>";
echo header("refresh: 5; url = ../");
exit();
